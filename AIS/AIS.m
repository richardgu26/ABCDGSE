...
function theta_s = sample_from_particles(particles, delta, lb, ub)
	...
	while !ok
		trial = theta_s;
       	trial(:,j) = trial(:,j) + 0.9*delta(:,j).*randn();
...
if node
	for iter = 1:iters;	
		...
		% number of draws
		if iter < iters
			ppn = particles_per_node;
		else
			ppn = particles_per_node2;
		endif	
		
		contribs = zeros(ppn, columns(particles));
		for i = 1:ppn
			ok = false;
			...
			while !ok
				if rand < mixture 
					asbil_theta = sample_from_prior(prior_params);
				else	
	...
	for iter = 1:iters	
		...
		% allow for more final round draws
		if iter == iters
			[particles, dist] = select_particles(Zn, oldparticles, particles, nparticles2, scale);
		else
			[particles, dist] = select_particles(Zn, oldparticles, particles, round(nparticles*particlequantile/100), scale);
		endif
